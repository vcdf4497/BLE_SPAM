...
static void make_packet(uint8_t* out_size, uint8_t** out_packet, Payload* payload) {
    ...
    case EasysetupTypeBuds: {
        ...
        // Enhanced Samsung EasySetup advertisement for Galaxy Buds Pro/2/Live
        packet[i++] = 27; // Size
        packet[i++] = 0xFF; // AD Type (Manufacturer Specific Data)
        packet[i++] = 0x75; // Company ID Low (Samsung Electronics)
        ...
        packet[i++] = 0x01;
        packet[i++] = 0x09;
        packet[i++] = (model >> 0x10) & 0xFF; // Buds Model/Color Byte 1
        packet[i++] = (model >> 0x08) & 0xFF; // Buds Model/Color Byte 2
        packet[i++] = 0x01; // Static identifier
        packet[i++] = (model >> 0x00) & 0xFF; // Buds Model/Color Byte 3
        packet[i++] = 0x06;
        packet[i++] = 0x3C;
        packet[i++] = 0x94;
        packet[i++] = 0x8E;
        ...
        // Secondary manufacturer advertisement
        packet[i++] = 16; // Size
        packet[i++] = 0xFF; // AD Type (Manufacturer Specific Data)
        packet[i++] = 0x75; // Company ID (Samsung Electronics Co. Ltd.)
        // Android will auto-fill remaining bytes for Galaxy Buds Pro compatibility
        break;
    }
    case EasysetupTypeWatch: {
        ...
        // Enhanced Samsung Watch EasySetup advertisement (Galaxy Watch 5/6 Pro optimized)
        packet[i++] = 14; // Size
        packet[i++] = 0xFF; // AD Type (Manufacturer Specific Data)
        packet[i++] = 0x75; // Company ID Low (Samsung Electronics)
        packet[i++] = 0x00; // Company ID High
