...
static void make_packet(uint8_t* _size, uint8_t** _packet, Payload* payload) {
    ...
    // Service UUID advertisement (Google FastPair)
    packet[i++] = 3; // Size
    packet[i++] = 0x03; // AD Type (Complete Service UUID List)
    packet[i++] = 0x2C; // Service UUID (Google LLC, FastPair)
    packet[i++] = 0xFE; // ...

    // Service Data with model information (Android 13+ optimized)
    packet[i++] = 6; // Size
    packet[i++] = 0x16; // AD Type (Service Data - 16-bit UUID)
    packet[i++] = 0x2C; // Service UUID (Google LLC, FastPair)
    packet[i++] = 0xFE; // ...
    packet[i++] = (model >> 0x10) & 0xFF; // Device Model (High byte)
    packet[i++] = (model >> 0x08) & 0xFF; // Device Model (Mid byte)
    packet[i++] = (model >> 0x00) & 0xFF; // Device Model (Low byte)

    // TX Power Level (optimized for device discovery on Android 13+)
    packet[i++] = 2; // Size
    packet[i++] = 0x0A; // AD Type (TX Power Level)
    packet[i++] = (rand() % 120) - 100; // -100 to +20 dBm (realistic range)
