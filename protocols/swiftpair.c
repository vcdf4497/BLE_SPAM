...
static void make_packet(uint8_t* _size, uint8_t** _packet, Payload* payload) {
    ...
    // Windows 11+ optimized SwiftPair BLE advertisement
    packet[i++] = size - 1; // Advertisement Data Length
    packet[i++] = 0xFF; // AD Type (Manufacturer Specific Data)
    packet[i++] = 0x06; // Company ID Low (Microsoft Corporation)
    packet[i++] = 0x00; // Company ID High
    packet[i++] = 0x03; // Microsoft Beacon ID (SwiftPair Signature)
    packet[i++] = 0x00; // Microsoft Beacon Sub-Scenario (Standard Pairing)
    packet[i++] = 0x80; // Flags: TX Power indicator byte (for Windows 11 detection)
    memcpy(&packet[i], name, name_len); // Device Name String
    i += name_len;
